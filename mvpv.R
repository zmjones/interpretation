...
pd.int = partial_dependence(fit, c("xpolity", "rgdppc"),
  n = c(17, nrow(dat[!dropped, ])), interaction = TRUE, uniform = TRUE)
pd.int = plot_transform(pd.int, c("xpolity", "rgdppc", outcome.names),
  c("xpolity", "rgdppc"))

## plots!
ggplot(pd.int[variable == "Deaths (One-Sided)", ], aes(rgdppc, value)) +
  geom_point() + geom_line() + 
  facet_wrap(~ xpolity, scales = "free") +
  labs(x = "Real GDP per Capita", y = "Repression")
ggsave("figures/pd_mvpv_int_repression.png", width = 10.5, height = 8)

ggplot(pd.int[variable == "Non-Violent Protest (count)", ], aes(rgdppc, value)) +
  geom_point() + geom_line() + facet_wrap(~ xpolity) +
  labs(x = "Real GDP per Capita", y = "Predicted ")
ggsave("figures/pd_mvpv_int_repression.png", width = 10.5, height = 8)
